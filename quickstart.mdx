...
In order to connect to these sources of knowledge, developers need to *authorize* with the application, *sync* data from the application's API, and finally *transform* the data into text-based documents that can be retrieved using LangChain or other LLM orchestration libraries.
