...
## Synchronization
Documents are synced from Notion workspaces to Psychic on a daily basis. These documents can be fetched from the `/get-documents` endpoint on an as-needed basis. Each call to this endpoint will fetch the most up to date documents from the Notion workspace associated with the provided `connection_id`. 
