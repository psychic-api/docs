...
## Synchronization
Documents are synced from Google Drive to Psychic on a daily basis. These documents can be fetched from the `/get-documents` endpoint on an as-needed basis. Each call to this endpoint will fetch the most up to date documents from the Google Drive folder associated with the provided `account_id`. 
